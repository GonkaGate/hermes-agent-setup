...
export function renderOnboardSuccess(result: OnboardSuccessResult): string {
  const lines = [
    ...
    "Saved settings:",
    `- providers.gonkagate.base_url = ${CANONICAL_BASE_URL}`,
    `- providers.gonkagate.key_env = ${GONKAGATE_API_KEY_ENV_VAR}`,
    "- providers.gonkagate.transport = chat_completions",
    "- providers.gonkagate.discover_models = false",
    "- providers.gonkagate.models = live GonkaGate models returned by /v1/models",
    ...
    `- Switch models with \`/model ${GONKAGATE_PROVIDER_SELECTOR}:${result.selectedModelId} --global\` or rerun the helper to pick another live GonkaGate model.`,
