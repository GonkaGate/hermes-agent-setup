...
Target upstream release: Hermes Agent `v2026.5.16` / Hermes `v0.14.0`
...
- The supported endpoint contract is `config.yaml` `model.provider`,
  `model.base_url`, and `model.default`.
- The supported secret contract is `.env` `OPENAI_API_KEY`.
...
- Current Hermes-owned surfaces that can still compete with the helper-managed
  endpoint remain safety checks: `model.api_key`, `model.api`,
  incompatible `model.api_mode`, matching auth pools, cron jobs with direct
  `base_url`, and shared `OPENAI_API_KEY` reuse surfaces.
...
- The helper still writes only `model.provider`, `model.base_url`,
  `model.default`, and `.env` `OPENAI_API_KEY` for the primary onboarding path.
